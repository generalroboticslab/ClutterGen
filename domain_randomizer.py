...
class DomainRandomizer:
    ...
    # Old version / Two stages randomization / Randomize movable objects on prefixed support surface
    ...
    # Totally randomization / Randomize movable objects / No prefixed support surface
    def fill_obj_poses(self, scene_dicts):
        for scene_dict in scene_dicts:
            movable_objs = scene_dict["movable"]

            obj_pos_min, obj_pos_max = self.to_torch([-2, -2, 0.]), self.to_torch([2., 2., 0.])
            obj_ori_min, obj_ori_max = self.to_torch([0., 0., -torch.pi]), self.to_torch([0., 0., torch.pi])
