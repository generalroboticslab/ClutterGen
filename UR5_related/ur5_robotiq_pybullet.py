...
from RRT.RRTPlan import RRTPlanner
...
def load_ur_robotiq_robot(robot_initial_pose, client_id=0):
    # load robot
    urdf_dir = '../ur5_robotiq_meta_package/ur5_robotiq_description/urdf'
    ...
    robot_id = p.loadURDF(urdf_filepath, basePosition=robot_initial_pose[0], baseOrientation=robot_initial_pose[1],
                          flags=p.URDF_USE_SELF_COLLISION, physicsClientId=client_id)
...
class UR5RobotiqPybulletController(object):
    ...
    def __init__(self, robot_id, rng=None, client_id=0):
        ...
        joint_infos = [p.getJointInfo(robot_id, joint_index, physicsClientId=self.client_id) for joint_index in range(p.getNumJoints(robot_id, physicsClientId=self.client_id))]
    ...
    def update_collision_check(self, target_id=[], disabled_collisions=[]):
        self.collision_fn_full = self.rrt.get_collision_fn(disabled_targets=disabled_collisions)  # recompute all collision combinations because scene change
        self.collision_fn_notarget = self.rrt.get_collision_fn(disabled_targets=target_id+disabled_collisions)
    ...
    def get_current_max_eef_velocity(self, arm_joint_values, client_id=0): # use pybullet to get max velocity
        ...
        eef_velocities = []
        for joint_value in arm_joint_values:
            eef_velocities.append(pu.get_max_velocity(self.id, joint_value, client_id=client_id))
        return eef_velocities
    ...
    def get_jacobian_pybullet(self, arm_joint_values):
        ...
        jac_t, jac_r = p.calculateJacobian(self.id, self.EEF_LINK_INDEX, (0, 0, 0),
                                           current_positions, zero_vec, zero_vec, physicsClientId=self.client_id)
    ...
    def get_arm_fk_pybullet(self, joint_values): # pybullet need to set joint then set back which pollutes visualization
        return pu.forward_kinematics(self.id, self.GROUP_INDEX['arm'], joint_values, self.EEF_LINK_INDEX, client_id=self.client_id)
    ...
    def get_arm_ik(self, pose_2d, timeout=0.1, restarts=1, avoid_collisions=True, arm_joint_values=None,
                   gripper_joint_values=None, solver='ikfast'): # default use ikfast, lift use pybullet

        start_joint_values = self.convert_range(self.get_arm_joint_values()) if arm_joint_values is None else self.convert_range(arm_joint_values) # ikfast return joints within [-pi, pi] that is the reason why we need to convert_range
        if solver == 'ikpybullet':
            return self.get_arm_ik_pybullet(pose_2d, avoid_collisions=avoid_collisions, arm_joint_values=start_joint_values)
        else: return self.get_ik_fast(pose_2d, avoid_collisions=avoid_collisions, arm_joint_values=start_joint_values)
    ...
    def get_arm_ik_pybullet(self, pose_2d, arm_joint_values=None, gripper_joint_values=None, avoid_collisions=True):
        ...
        if ik_result is not None:
            ik_result = self.convert_range(ik_result) # convert range here!
            if avoid_collisions:
    ...
    def random_initial(self, box_ext=0.003):
        ...
        while res is None:
            ...
            res = self.get_arm_ik([new_position, new_orientation], avoid_collisions=True)
            reset_try_times += 1
