...
class PostCorrector:
    def __init__(self, handem_env, gen_readable_table=True, device="cuda") -> None:
        self.handem_env = handem_env
        self.device = device

        self.force_threshold = 20
        self.vel_threshold = [0.02, torch.pi/5] # Translation, rotation speed
        self.gen_readable_table = gen_readable_table
    ...
    def handed_check_realiablity(self, env_info, cur_scene_dict, force_check=True, vel_check=True):
        ...
        if self.gen_readable_table:
            ...
            # for j in range(len(object_force)):
            #     if body_ids[j] not in force_fail_obj_ids:
            #         data.append(["FORCE", env_ids[j], body_names[j], body_ids[j], object_force[j]])
