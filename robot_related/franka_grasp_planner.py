...
class FrankaGraspLabeler:
    OPEN_POSITION = [0.04] * 2
    CLOSED_POSITION = 0. * np.array([1, 1])
    GROUPS = {
    ...
    def load_gripper(self):
        self.gripper_id = self.loadURDF("robot_related/franka_description/robots/franka_panda_gripper.urdf", 
                                        basePosition=[0, 0, 0.], 
                                        useFixedBase=True)
        ...
        for k, v in events.items():
            if v & p.KEY_WAS_TRIGGERED:
                ...
                if k == ord('r'):
                    ...
                    C2GB_rpy[0] = (C2GB_rpy[0] + pi) % (2*pi)
                elif k == ord('e'):
                    C2GB_rpy[0] = (C2GB_rpy[0] - pi/36) % (2*pi)
                elif k == ord('a'):
                    C2GB_rpy[1] = (C2GB_rpy[1] + pi/36) % (2*pi)
                elif k == ord('2'):
                    C2GB_rpy[1] = (C2GB_rpy[1] + pi) % (2*pi)
                elif k == ord('d'):
                    C2GB_rpy[1] = (C2GB_rpy[1] - pi/36) % (2*pi)
                elif k == ord('z'):
                    C2GB_rpy[2] = (C2GB_rpy[2] + pi/36) % (2*pi)
                elif k == ord('3'):
                    C2GB_rpy[2] = (C2GB_rpy[2] + pi) % (2*pi)
                elif k == ord('c'):
                    C2GB_rpy[2] = (C2GB_rpy[2] - pi/36) % (2*pi)
...
if __name__ == "__main__":
    asset_root = "assets"
    group_name = "group4_real_objects"
    obj_labeler = FrankaGraspLabeler(asset_root, group_name=group_name)
    obj_labeler.start()